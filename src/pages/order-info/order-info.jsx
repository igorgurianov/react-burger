...
const OrderInfo = ({ style }) => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const { order, success, isLoading } = useSelector(
      ...
      >
        <div className={styles.content}>
          ...
          <ul className={`${styles.list} mt-6 custom-scroll`}>
            {order.ingredients &&
              countIngredients(order.ingredients).map((ingredient, index) => {
                return (
          ...
          <div className={`${styles.info} mt-10`}>
            ...
            <div className={styles.total}>
              <span className="text text_type_digits-default mr-2">
                {countIngredients(order.ingredients).reduce((acc, item) => {
